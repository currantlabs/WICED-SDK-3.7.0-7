    ...
    switch (i)
    {
        ...
        case APOLLO_CONSOLE_CMD_STATS:
            if (g_apollo->player_handle)
            {
                if (apollo_player_ioctl(g_apollo->player_handle, APOLLO_PLAYER_IOCTL_GET_SOCKET, &socket) == WICED_SUCCESS)
                {
                    ...
                    {
                        apollo_log_printf("Player UDP: packets sent %u, bytes sent %u, packets received %u, bytes received %u\n", packets_sent, bytes_sent, packets_received, bytes_received);
                        apollo_log_printf("            packets queued %u, receive packets dropped %u, checksum errors %u\n", packets_queued, receive_packets_dropped, checksum_errors);
                    }
                }
                ...
                if (apollo_player_ioctl(g_apollo->player_handle, APOLLO_PLAYER_IOCTL_GET_STATS, &stats) == WICED_SUCCESS)
                {
                    ...
                    apollo_log_printf("              audio frames played %lu, audio frames dropped %lu, audio frames inserted %lu\n",
                ...
                apollo_log_printf("Apollo stats:  total packets received %lu, total packets dropped %lu\n",
                ...
                apollo_log_printf("               total audio frames played %lu, total audio dropped %lu, total frames inserted %lu\n",
        ...
        case APOLLO_CONSOLE_CMD_NETLOG:
            if (!strcmp(argv[1], "on"))
            {
                ...
                {
                    apollo_log_msg(APOLLO_LOG_ERR, "Network logging is already active\n");
                }
            }
        ...
        case APOLLO_CONSOLE_CMD_RMC_STATUS:
        {
            ...
            {
                apollo_log_printf("  %02x:%02x:%02x:%02x:%02x:%02x  %3d  %c %c %c %c\r\n",
...
static int apollo_player_event_callback(apollo_player_ref handle, void* userdata, APOLLO_PLAYER_EVENT_T event, void* arg)
{
    ...
    switch (event)
    {
        ...
        case APOLLO_PLAYER_EVENT_RTP_TIMING_FULL:
            apollo_log_msg(APOLLO_LOG_ERR, "RTP timing log is full\n");
            break;
    }
...
#endif
            apollo_log_msg(APOLLO_LOG_INFO, "BT track: %lu / %lu secs\n", position, duration);
        }
            break;
...
wiced_result_t gatt_event_callback(apollo_config_gatt_event_t event, apollo_config_gatt_server_dct_t *dct,  void *user_context)
{
    ...
    switch ( event)
    {
        ...
        case APOLLO_CONFIG_GATT_EVENT_DCT_WRITE:
            ...
            memset((char *)apollo->dct_tables.dct_wifi->stored_ap_list[0].details.SSID.value, 0,
                    sizeof(apollo->dct_tables.dct_wifi->stored_ap_list[0].details.SSID.value));
            apollo->dct_tables.dct_wifi->stored_ap_list[0].details.SSID.length = strlen(dct->nw_ssid_name);
            strlcpy((char *)apollo->dct_tables.dct_wifi->stored_ap_list[0].details.SSID.value,
                    dct->nw_ssid_name,
                    sizeof(apollo->dct_tables.dct_wifi->stored_ap_list[0].details.SSID.value));

            memset((char *)apollo->dct_tables.dct_wifi->stored_ap_list[0].security_key, 0,
                    sizeof(apollo->dct_tables.dct_wifi->stored_ap_list[0].security_key));
            apollo->dct_tables.dct_wifi->stored_ap_list[0].security_key_length = strlen(dct->nw_pass_phrase);
            strlcpy((char *)apollo->dct_tables.dct_wifi->stored_ap_list[0].security_key,
                    dct->nw_pass_phrase,
                    sizeof(apollo->dct_tables.dct_wifi->stored_ap_list[0].security_key));

#ifdef WICED_DCT_INCLUDE_BT_CONFIG
            memset((char *)apollo->dct_tables.dct_bt->bluetooth_device_name, 0,
                    sizeof(apollo->dct_tables.dct_bt->bluetooth_device_name));
            strlcpy((char *)apollo->dct_tables.dct_bt->bluetooth_device_name,
                    dct->nw_ssid_name,
                    sizeof(apollo->dct_tables.dct_bt->bluetooth_device_name));
#endif
            break;
...
static void apollo_handle_timer(apollo_app_t* apollo)
{
    ...
    {
        apollo_log_msg(APOLLO_LOG_ERR, "Error querying player stats\n");
    ...
    {
        apollo_log_msg(APOLLO_LOG_ERR, "Unable to allocate report packet\n");
        return;
    }
    ...
    if (result != WICED_SUCCESS)
    {
        ...
        apollo_log_msg(APOLLO_LOG_ERR, "Error sending report packet\n");
...
static void apollo_handle_rtp_timing_cmd(apollo_app_t* apollo)
{
    ...
    switch (apollo->rtp_timing_cmd)
    {
        case APOLLO_CMD_RTP_TIMING_INIT:
            ...
            {
                apollo_log_msg(APOLLO_LOG_ERR, "Error initializing RTP timing log with %u entries\n", apollo->rtp_timing_entries);
            }
            break;
        ...
        case APOLLO_CMD_RTP_TIMING_START:
            ...
            {
                apollo_log_msg(APOLLO_LOG_ERR, "Error starting RTP timing log\n");
            }
            break;
        ...
        case APOLLO_CMD_RTP_TIMING_STOP:
            ...
            {
                apollo_log_msg(APOLLO_LOG_ERR, "Error stopping RTP timing log\n");
            }
            break;
        ...
        case APOLLO_CMD_RTP_TIMING_RESET:
            ...
            {
                apollo_log_msg(APOLLO_LOG_ERR, "Error resetting RTP timing log\n");
            }
            break;
        ...
        case APOLLO_CMD_RTP_TIMING_DUMP:
            ...
            {
                apollo_log_msg(APOLLO_LOG_ERR, "Error getting RTP timing log data\n");
                break;
            }
            ...
            apollo_log_printf("RTP Timing Log Dump:\n");
            for (i = 0; i < rtp_timing.number_entries; i++)
            {
                apollo_log_printf("  Seq: %04u  arrival time: %d.%09d\n", rtp_timing.rtp_entries[i].sequence_number,
        ...
        default:
            apollo_log_msg(APOLLO_LOG_ERR, "Unknown RTP timing log command: %d\n", apollo->rtp_timing_cmd);
            break;
    }
...
static void apollo_mainloop(apollo_app_t* apollo)
{
    ...
    while (apollo->tag == APOLLO_TAG_VALID)
    {
        ...
        {
            apollo_log_msg(APOLLO_LOG_WARNING, "Saving new BTLE GATT attributes and rebooting...\r\n");
        ...
        case APOLLO_CMD_EVENT_SET_LOG_SERVER:
            ...
            if (ip_addr.ip.v4 != 0 && !apollo->network_logging_active)
            {
                if (apollo_debug_create_tcp_data_socket(&ip_addr, 0) == WICED_SUCCESS)
                {
                    ...
                    apollo_log_msg(APOLLO_LOG_ERR, "Apollo IP address: %d.%d.%d.%d   MAC: %02x:%02x:%02x:%02x:%02x:%02x\n",
...
static apollo_app_t* apollo_init(void)
{
    ...
    if (result != WICED_SUCCESS)
    {
        apollo_log_msg(APOLLO_LOG_ERR, "Error initializing timer\n");
        tag = APOLLO_TAG_INVALID;
    }
    ...
    {
        apollo_log_msg(APOLLO_LOG_ERR, "Can't start main BT service !\r\n");
    }
    ...
    if (result != WICED_SUCCESS)
    {
        apollo_log_msg(APOLLO_LOG_ERR, "Can't start BT GATT service !\r\n");
    }
#endif
    ...
    {
        result = apollo_network_up_default(&interface, WICED_USE_INTERNAL_DHCP_SERVER,
                                           &ap_ip_settings, apollo->dct_tables.dct_app->apollo_role);
    }
    else
    {
        result = apollo_network_up_default(&interface, WICED_USE_EXTERNAL_DHCP_SERVER,
                                           NULL, apollo->dct_tables.dct_app->apollo_role);
    }
    ...
    apollo_log_msg(APOLLO_LOG_INFO, "Creating Apollo report socket\n");
    result = wiced_udp_create_socket(&apollo->report_socket, APOLLO_REPORT_PORT + 1, apollo->dct_tables.dct_network->interface);
    if (result != WICED_SUCCESS)
    {
        apollo_log_msg(APOLLO_LOG_ERR, "Unable to create report socket\n");
    }
