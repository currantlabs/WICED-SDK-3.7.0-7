...
static void apollo_config_command_help(void)
{
    ...
    apollo_log_printf("    config burst_length <count> | auto  : from %d to %d\r\n", 0, RTP_AUDIO_BL_MAX_LENGTH);
    apollo_log_printf("    config bl           <packet_count>\r\n" );
    apollo_log_printf("    config shuffle_length <count>       : from %d to %d\r\n", 0, RTP_AUDIO_BL_MAX_LENGTH);
    apollo_log_printf("    config sl           <packet_count>\r\n" );
    ...
    switch(cmd)
    {
        ...
        case CONFIG_CMD_BURST_LENGTH:
        {
            ...
            if (argc > 2)
            {
                ...
                {
                    apollo_log_msg(APOLLO_LOG_DEBUG0, "Burst length: %d -> %d\r\n", dct_tables->dct_app->burst_length, burst_length);
                    dct_tables->dct_app->burst_length = burst_length;
            ...
            {
                apollo_log_msg(APOLLO_LOG_NOTICE, "Burst length: %d\r\n", dct_tables->dct_app->burst_length);
            }
            else
            {
                apollo_log_msg(APOLLO_LOG_NOTICE, "Burst length: auto\r\n");
            }
            break;
        }
