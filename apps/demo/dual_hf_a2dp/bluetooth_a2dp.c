...
wiced_result_t bt_audio_a2dp_send_passthru_command(uint8_t cmd, int index);
...
bt_audio_a2dp_context_data_t a2dp_context[BT_A2DP_CONTEXT_MAX_LIMIT] =
{
    {
...
int a2dp_context_index = 0;
volatile int current_streaming_peer_index = 0xf;
volatile int a2dp_sink_start_pending_index = 0xf;
volatile wiced_bool_t a2dp_sink_start_pending = WICED_FALSE;
...
void bt_audio_a2dp_disconnect_service(void)
{
    ...
    result = wiced_bt_a2dp_sink_disconnect(a2dp_context[a2dp_context_index].a2dp_peer_address);
    if( WICED_SUCCESS != result )
    {
...
void bt_audio_avrcp_disconnect_service(void)
{
    ...
    result = wiced_bt_remote_control_disconnect(a2dp_context[a2dp_context_index].avrc_peer_address);
    if( WICED_SUCCESS != result )
    {
...
static void bt_audio_app_sink_avrc_connect(void)
{
    if(a2dp_context[a2dp_context_index].rc_connected == WICED_FALSE && a2dp_context[a2dp_context_index].sink_connected == WICED_TRUE )
    {
        wiced_bt_remote_control_connect(a2dp_context[a2dp_context_index].a2dp_peer_address);
    }
    return;
}
    ...
    switch(event)
    {
        case WICED_BT_A2DP_SINK_CONNECT_EVT:
            ...
            if(p_data->connect.result == WICED_SUCCESS)
            {
                ...
                a2dp_context_index = set_a2dp_context(bda);
                if (a2dp_context_index >= 0 && a2dp_context_index < BT_A2DP_CONTEXT_MAX_LIMIT)
                {
                    a2dp_context[a2dp_context_index].sink_connected = WICED_TRUE;
                    memcpy( a2dp_context[a2dp_context_index].a2dp_peer_address, p_data->connect.bd_addr, sizeof(wiced_bt_device_address_t) );
                    memcpy( &a2dp_context[a2dp_context_index].decoder_config, &codec_config, sizeof( wiced_bt_a2dp_codec_info_t ) );
                    bt_audio_get_config_from_cie(&a2dp_context[a2dp_context_index].decoder_config, &a2dp_context[a2dp_context_index].audio_config);
                }
                service = wiced_get_entry(SERVICE_BT_A2DP);
                service->state = SERVICE_ENABLED;
                if(a2dp_context[a2dp_context_index].rc_connected == WICED_FALSE)
                {
            ...
            {
                a2dp_context[a2dp_context_index].sink_connected = WICED_FALSE;
                signal_for_event_semaphore( WICED_BT_CONNECT_A2DP, WICED_FALSE );
            }
#ifdef USE_AUDIO_DISPLAY
        ...
        case WICED_BT_A2DP_SINK_DISCONNECT_EVT:
        {
            ...
            a2dp_context_index = get_a2dp_context(bda);
            if (a2dp_context_index >= 0 && a2dp_context_index < BT_A2DP_CONTEXT_MAX_LIMIT)
            {
                a2dp_context[a2dp_context_index].sink_connected = WICED_FALSE;
                memset( a2dp_context[a2dp_context_index].a2dp_peer_address, 0, sizeof(wiced_bt_device_address_t) );
                memset( &a2dp_context[a2dp_context_index].decoder_config, 0, sizeof( wiced_bt_a2dp_codec_info_t ) );
                if(!memcmp( a2dp_context[a2dp_context_index].avrc_peer_address, null_addr, sizeof(wiced_bt_device_address_t) ) )
                {
                    a2dp_context[a2dp_context_index].rc_connected = WICED_FALSE;
                    a2dp_context[a2dp_context_index].in_use_context = WICED_FALSE;
                }
            }
        ...
        case WICED_BT_A2DP_SINK_START_EVT:
        {
            ...
            WPRINT_APP_INFO( ("bt_audio_sink_control_cb:STREAM START EVENT status = %d \n Remote Bluetooth Address: [%02X:%02X:%02X:%02X:%02X:%02X]\n",
                            p_data->start.result, bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]));
            WPRINT_APP_INFO( ("bt_audio_sink_control_cb:STREAM START EVENT before current_streaming_peer_index:%x, a2dp_sink_start_pending:%x\n", current_streaming_peer_index, a2dp_sink_start_pending) );
            a2dp_context_index = get_a2dp_context(bda);
            if (a2dp_context_index >= 0 && a2dp_context_index < BT_A2DP_CONTEXT_MAX_LIMIT)
            {
                service = wiced_get_entry(SERVICE_BT_A2DP);
                ...
                if(current_streaming_peer_index == 0xf)  /*Previously no device were streaming*/
                {
                    /* Configure codec and player(DAC) with the configuration received thru Codec Config event from the peer */
                    bt_audio_configure_decoder(&a2dp_context[a2dp_context_index].decoder_config);
                    bt_audio_configure_device( &a2dp_context[a2dp_context_index].audio_config, BT_AUDIO_DEVICE_PLAYER );
                    app_set_current_service(SERVICE_BT_A2DP);
                    app_set_service_state(SERVICE_BT_A2DP, SERVICE_PLAYING_AUDIO);
                    current_streaming_peer_index = a2dp_context_index;
                    app_data.a2dp_media_state = WICED_TRUE;
                    app_data.play_button_state = WICED_TRUE;
                    a2dp_sink_start_pending = WICED_FALSE;
                    WPRINT_APP_INFO( ("[BT-A2DP]: Music streaming from the phone\n") );
                }
                else if (current_streaming_peer_index != a2dp_context_index) /* START request from different device*/
                {
                    WPRINT_APP_INFO( ( "sending PAUSE to current streaming peer\n" ) );
                    bt_audio_a2dp_send_passthru_command( AVRC_ID_PAUSE, current_streaming_peer_index);
                    ...
                    app_data.play_button_state = WICED_FALSE;
                    a2dp_sink_start_pending = WICED_TRUE;
                    a2dp_sink_start_pending_index = a2dp_context_index;
                }
            }
            WPRINT_APP_INFO( ("bt_audio_sink_control_cb:STREAM START EVENT After current_streaming_peer_index:%x, a2dp_sink_start_pending:%x\n", current_streaming_peer_index, a2dp_sink_start_pending) );
        }
        break;
        ...
        case WICED_BT_A2DP_SINK_SUSPEND_EVT:
            ...
            WPRINT_APP_INFO( ("bt_audio_sink_control_cb:STREAM SUSPEND EVENT before current_streaming_peer_index:%d, a2dp_sink_start_pending:%x\n", current_streaming_peer_index, a2dp_sink_start_pending) );
            a2dp_context_index = get_a2dp_context(bda);
            if (a2dp_context_index >= 0 && a2dp_context_index < BT_A2DP_CONTEXT_MAX_LIMIT)
            {
                if(a2dp_sink_start_pending == WICED_FALSE)
                {
                    current_streaming_peer_index = 0xf;
                    bt_audio_end_loop(BT_AUDIO_DEVICE_PLAYER);
                    bt_audio_reset_decoder_config();
                    app_reset_current_service(SERVICE_BT_A2DP);
                    app_set_service_state(SERVICE_BT_A2DP, SERVICE_PAUSED_AUDIO);
                ...
                {
                    bt_audio_configure_decoder(&a2dp_context[a2dp_sink_start_pending_index].decoder_config);
                    bt_audio_configure_device( &a2dp_context[a2dp_sink_start_pending_index].audio_config, BT_AUDIO_DEVICE_PLAYER );
                    app_set_current_service(SERVICE_BT_A2DP);
                    app_set_service_state(SERVICE_BT_A2DP, SERVICE_PLAYING_AUDIO);
                    current_streaming_peer_index = a2dp_sink_start_pending_index;
                    app_data.a2dp_media_state = WICED_TRUE;
                    app_data.play_button_state = WICED_TRUE;
                    a2dp_sink_start_pending = WICED_FALSE;
                    /*saving a2dp_context_index since it is being used some cases where bdaddr is not available */
                    a2dp_context_index = current_streaming_peer_index;
                }
            }
            WPRINT_APP_INFO( ("bt_audio_sink_control_cb:STREAM SUSPEND EVENT AFTER current_streaming_peer_index:%x, a2dp_sink_start_pending:%x\n", current_streaming_peer_index, a2dp_sink_start_pending) );
#ifdef USE_MEM_POOL
...
static void bt_audio_sink_data_cb( wiced_bt_a2dp_sink_codec_t codec_type, wiced_bt_a2dp_sink_audio_data_t* p_audio_data )
{
    ...
    if(a2dp_sink_start_pending == WICED_TRUE)
    {
        WPRINT_APP_INFO( ( "dropping the packet\n" ) );
        return;
    }
                                                                         ...
                                                                         wiced_bt_remote_control_connection_state_t connection_state, uint32_t peer_features)
{
    ...
    {
        a2dp_context_index = set_a2dp_context(remote_addr);
        if (a2dp_context_index >= 0 && a2dp_context_index < BT_A2DP_CONTEXT_MAX_LIMIT)
        {
            WPRINT_APP_INFO( ("%s: a2dp_context_index = %d \n", __func__, a2dp_context_index) );
        }
        else
        {
            WPRINT_APP_INFO( ("%s: Invalid Index a2dp_context_index = %d \n", __func__, a2dp_context_index) );
            return;
        }
        if(!memcmp( a2dp_context[a2dp_context_index].avrc_peer_address, null_addr, sizeof(wiced_bt_device_address_t) ) )
        {
            memcpy(a2dp_context[a2dp_context_index].avrc_peer_address, remote_addr, sizeof(wiced_bt_device_address_t) ) ;
        }
        else
        {
            wiced_assert( "peer bd adress for a2dp and avrcp do not match!\n", ( !memcmp(remote_addr, a2dp_context[a2dp_context_index].avrc_peer_address, sizeof(wiced_bt_device_address_t)) ) );
            if( memcmp(remote_addr, a2dp_context[a2dp_context_index].avrc_peer_address, sizeof(wiced_bt_device_address_t) ))
            {
                if(!memcmp( a2dp_context[a2dp_context_index].a2dp_peer_address, null_addr, sizeof(wiced_bt_device_address_t) ))
                {
                    a2dp_context[a2dp_context_index].sink_connected = WICED_FALSE;
                    a2dp_context[a2dp_context_index].in_use_context = WICED_FALSE;
                }
        ...
        a2dp_context[a2dp_context_index].peer_features = peer_features;
        a2dp_context[a2dp_context_index].rc_connected = WICED_TRUE;
    ...
    {
        a2dp_context_index = get_a2dp_context(remote_addr);
        if (a2dp_context_index >= 0 && a2dp_context_index < BT_A2DP_CONTEXT_MAX_LIMIT)
        {
            WPRINT_APP_INFO( ("%s: a2dp_context_index = %d \n", __func__, a2dp_context_index) );
        }
        ...
        memset( a2dp_context[a2dp_context_index].avrc_peer_address, 0, sizeof( wiced_bt_device_address_t ) );
        a2dp_context[a2dp_context_index].peer_features = 0;
        a2dp_context[a2dp_context_index].rc_connected = WICED_FALSE;
        if(!memcmp( a2dp_context[a2dp_context_index].a2dp_peer_address, null_addr, sizeof(wiced_bt_device_address_t) ) )
        {
            a2dp_context[a2dp_context_index].sink_connected = WICED_FALSE;
...
static void bt_audio_remote_control_rsp_cback(wiced_bt_device_address_t remote_addr, wiced_bt_avrc_response_t *avrc_rsp)
{
    ...
    WPRINT_APP_INFO( ("bt_audio_remote_control_rsp_cback current_streaming_peer_index:%d\n", current_streaming_peer_index) );
...
wiced_result_t bt_audio_a2dp_send_passthru_command(uint8_t cmd, int index)
{
    ...
    WPRINT_APP_INFO( ("bt_audio_a2dp_send_passthru_command: [%02X:%02X:%02X:%02X:%02X:%02X]\n",
                    a2dp_context[index].avrc_peer_address[0], a2dp_context[index].avrc_peer_address[1], a2dp_context[index].avrc_peer_address[2],
                    a2dp_context[index].avrc_peer_address[3], a2dp_context[index].avrc_peer_address[4], a2dp_context[index].avrc_peer_address[5]));
    WPRINT_APP_INFO( ("bt_audio_a2dp_send_passthru_command: cmd:%x, index:%d", cmd, index));

    result = wiced_bt_remote_control_send_pass_through_cmd( a2dp_context[index].avrc_peer_address, cmd, AVRC_STATE_PRESS, 0, NULL );
    if(result != WICED_SUCCESS)
        return result;
    wiced_rtos_delay_milliseconds( 50 );
    result = wiced_bt_remote_control_send_pass_through_cmd( a2dp_context[index].avrc_peer_address, cmd, AVRC_STATE_RELEASE, 0, NULL );
    return result;
}
...
static wiced_result_t bt_audio_a2dp_skip_backward( void )
{
    ...
    return bt_audio_a2dp_send_passthru_command( AVRC_ID_BACKWARD ,a2dp_context_index );
}
...
static wiced_result_t bt_audio_a2dp_pause_play(void)
{
    ...
    if( app_data.play_button_state == WICED_TRUE )
    {
        ...
        result = bt_audio_a2dp_send_passthru_command(AVRC_ID_PAUSE, a2dp_context_index );
        if(result == WICED_SUCCESS)
            app_data.play_button_state = WICED_FALSE;
    }
    else
    {
        ...
        result = bt_audio_a2dp_send_passthru_command(AVRC_ID_PLAY, a2dp_context_index );
        if(result == WICED_SUCCESS)
            app_data.play_button_state = WICED_TRUE;
    }
...
static wiced_result_t bt_audio_a2dp_update_volume( uint8_t vol_level )
{
    ...
    result = wiced_bt_remote_control_set_volume_cmd(a2dp_context[a2dp_context_index].avrc_peer_address, vol_level);
    return result;
}

static wiced_result_t bt_audio_a2dp_volume_up( void )
{
    uint8_t vol_level = a2dp_context[a2dp_context_index].audio_config.volume;
    wiced_result_t result;
    ...
    a2dp_context[a2dp_context_index].audio_config.volume = vol_level;
...
static wiced_result_t bt_audio_a2dp_stop( void )
{
    return bt_audio_a2dp_send_passthru_command(AVRC_ID_STOP, a2dp_context_index);
}
...
static wiced_result_t bt_audio_a2dp_skip_forward( void )
{
    ...
    return bt_audio_a2dp_send_passthru_command(AVRC_ID_FORWARD, a2dp_context_index);
}
...
static void bt_audio_a2dp_prevent_service( wiced_app_service_t *service )
{
    ...
    if( service->state == SERVICE_PLAYING_AUDIO )
    {
        bt_audio_a2dp_send_passthru_command(AVRC_ID_PAUSE, a2dp_context_index);
        wiced_bt_a2dp_sink_suspend( a2dp_context[a2dp_context_index].a2dp_peer_address);
