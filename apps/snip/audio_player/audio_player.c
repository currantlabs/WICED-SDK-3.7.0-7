...
#define AUDIO_PLAYER_CONSOLE_COMMANDS \
    ...
    { (char*) "apollo_log",     audio_player_console_command,    0, NULL, NULL, (char *)"", (char *)"Set apollo log level" }, \
...
typedef enum
{
    ...
    AUDIO_PLAYER_CONSOLE_CMD_APOLLO_LOG_LEVEL,
...
static int render_log_output(APOLLO_LOG_LEVEL_T level, char *logmsg)
{
...
static int audio_player_console_command(int argc, char *argv[])
{
    ...
    switch (i)
    {
        ...
        case AUDIO_PLAYER_CONSOLE_CMD_APOLLO_LOG_LEVEL:
            apollo_log_set_level(atoi(argv[1]));
            break;
    }
...
static audio_player_t *init_player(void)
{
    ...
    apollo_log_init(APOLLO_LOG_ERR, render_log_output);    /* For audio render logging messages */
    ...
    /* App */
    wiced_dct_read_lock( (void**)&player->dct_app, WICED_TRUE, DCT_APP_SECTION, 0, sizeof( audio_player_app_dct_t ) );
