...
static wiced_result_t audio_player_save_network_dct(audio_player_t* player);
static wiced_result_t audio_player_save_wifi_dct(audio_player_t* player);
...
static wiced_bool_t audio_player_get_channel_band(int channel, int* band);
...
static wiced_security_t audio_player_parse_security_type(char* security_str)
{
    int table_index;

    for (table_index = 0; table_index < sizeof(audio_player_security_name_table)/sizeof(security_lookup_t); table_index++)
    {
        if (strcasecmp(audio_player_security_name_table[table_index].name, security_str) == 0)
        {
            return audio_player_security_name_table[table_index].sec_type;
        }
    }
...
static wiced_bool_t audio_player_parse_mac_addr(char* in, wiced_mac_t* mac)
{
    char* colon;
    ...
    switch( cmd )
    {
        ...
        case CONFIG_CMD_MAC_ADDR:
        {
            ...
            if (argc > 2)
            {
                ...
                if (audio_player_parse_mac_addr(argv[2], &new_mac) != WICED_TRUE)
                {
                ...
                if (memcmp(&new_mac, &player->dct_wifi->mac_address, sizeof(wiced_mac_t)) != 0)
                {
                    ...
                    audio_player_get_mac_addr_text(&new_mac, new_mac_str, sizeof(new_mac_str));
...

            {
                ...
                audio_player_get_mac_addr_text(&wiced_mac, mac_str, sizeof(mac_str));
        ...
        case CONFIG_CMD_NETWORK_CHANNEL:
        {
            ...
            if (argc > 2)
            {
                ...
                if (audio_player_get_channel_band(new_channel, &new_band) != WICED_TRUE)
                {
        ...
        case CONFIG_CMD_NETWORK_SECURITY_TYPE:
            if (argc > 2)
            {
                ...
                new_sec_type = audio_player_parse_security_type(argv[2]);
                ...
                if (player->dct_wifi->stored_ap_list[0].details.security != new_sec_type)
                {
                    WPRINT_APP_DEBUG(("network security %s -> %s\r\n",
                            audio_player_get_security_type_name( player->dct_wifi->stored_ap_list[0].details.security),
                            audio_player_get_security_type_name(new_sec_type)));
        ...
        case CONFIG_CMD_SAVE:
            ...
            if (net_dct_dirty != 0)
            {
                ...
                audio_player_save_network_dct(player);
                net_dct_dirty = 0;
            ...
            if (wifi_dct_dirty != 0)
            {
                ...
                audio_player_save_wifi_dct(player);
...
static void audio_player_print_wifi_info(audio_player_t* player)
{
    ...
    if (player->dct_network->interface == WICED_STA_INTERFACE)
    {
        ...
        WPRINT_APP_INFO(("            SSID: %.*s %c\r\n", player->dct_wifi->stored_ap_list[0].details.SSID.length,
                                                        ...
                                                        (wifi_dct_dirty & WIFI_DCT_SSID_DIRTY) ? '*' : ' '));
        WPRINT_APP_INFO(("        Security: %s %c\r\n", audio_player_get_security_type_name(sec), (wifi_dct_dirty & WIFI_DCT_SECURITY_TYPE_DIRTY) ? '*' : ' '));
